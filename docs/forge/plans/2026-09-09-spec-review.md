...
- a span ending in an extension carried only by a newly tracked file classifies as a path
- hardcoding the extension set makes that test fail
...
- `python3 -c "import sys,inspect; sys.path.insert(0,'scripts'); import forge_docreview as d; p=inspect.signature(d.validate_verdict).parameters['spec_sections']; assert p.default is inspect.Parameter.empty, 'spec_sections has a default: %r' % (p.default,)"` — the enforcement input has no default of any kind, checked by inspecting the signature rather than by grepping for one spelling of one default value
...
### Task 7: Make the gate invokable
- [ ] Done

**Files:**
- Modify: `skills/brainstorming/SKILL.md` (step 8 gains the invocation contract)
- Modify: `scripts/forge_docreview.py` (drop `--section` and scoped packets)
- Modify: `tests/test_forge_docreview.py`
- Modify: `tests/test_forge_docs.py`

**Spec:** Spec review, Brainstorming flow contracts

**Interface:** step 8 states how the gate is actually run, at the register `execution.md` uses for task-reviewer dispatch and no heavier — this is a single-shot gate, not a multi-attempt loop, so it needs no tier routing, rework loop or convergence rule. It names: the packet is generated by `forge_docreview.py --spec <path> --out <dir>`; the reviewer is a **fresh** agent whose prompt is the packet file path; its verdict is captured to a file; `forge_docreview.py --spec <path> --verdict <file>` validates and disposes it; a non-zero exit is an invalid verdict, which the **caller** re-invokes once with the named defect before it becomes a contract error. Scoped review is removed rather than defined: `--section`, `build_packet`'s `sections` parameter, and `_scoped_reference_table` are deleted, and an amendment re-reviews the whole document (`pipeline` spec, amended). `validate_verdict`'s `spec_sections` argument stays required and is populated from the spec's own headings. The top-level `verdict` key is required, checked against its enumeration, and checked for self-consistency — `pass` carries no findings, `findings` carries at least one. It is currently neither validated nor read: a verdict omitting it, naming a value outside the enumeration, or contradicting its own findings list all validate clean today. Top-level scalars enter the same declared-schema mechanism that governs the entry arrays rather than getting a bespoke check, so the class is closed rather than the instance patched. Symbol resolution matches on **word boundaries**, so a substring of a longer identifier does not satisfy a claim — `REPO` must not be accepted for a file containing only `REPO_ROOT`. Its residual limit is recorded in place rather than implied: a fixed-string search cannot tell a symbol used in code from a word appearing in a comment or docstring, and distinguishing them needs per-language parsing that `stdlib-only` puts out of reach. A span containing whitespace or a quote character is not a path reference — spec prose carries shell commands and JSON fragments that the slash rule otherwise admits. The packet must also STATE the envelope: a fresh reviewer's entire prompt is the packet path, so a field the packet omits is a field the reviewer cannot know to emit.

**Tests:**
- step 8 names the packet-generation command
- step 8 states the reviewer is a fresh agent and the packet path is its prompt
- step 8 states how a verdict is validated and what a non-zero exit means
- step 8 states the caller owns the single retry
- step 8 does not describe tier routing, a rework loop, or convergence
- the CLI rejects `--section` as an unknown argument
- `build_packet` takes no sections parameter
- a packet for a spec always contains every section of that spec
- no scoped-packet helper remains in the module
- `spec_sections` is still required and still has no default
- a verdict with no top-level `verdict` key is invalid
- a `verdict` value outside pass or findings is invalid
- a `verdict` of pass carrying findings is invalid, and findings carrying none is invalid
- the packet states the top-level verdict envelope and that the verdict is JSON written to a file
- a symbol matching only as a substring of a longer identifier does not satisfy a dependency claim
- a span containing whitespace or a quote character is not classified as a path
- a shell command or JSON fragment in spec prose is dropped from the reference table

**Acceptance:**
- `python3 -m pytest tests/ -q` passes with no skips introduced by this task
- `python3 scripts/forge_docreview.py --spec docs/forge/specs/codex-runner.md --section X; [ $? -ne 0 ]` — `--section` is gone
- `! grep -n '_scoped_reference_table\|sections=None' scripts/forge_docreview.py` — the scoped path is deleted, not merely unreachable
- `python3 scripts/forge_lint.py --specs` exits 0
