...
**Follow-up:** after a few real runs on the resumed-worker loop, revisit `scripts/forge-monitor.py` and the runner's event→line mapping together — what a discovery review, a batched repair, and a resumed verification should each look like in the panel. Likely candidates: collapsing reasoning noise, marking cold-spawn vs. resume, and surfacing the coverage checklist's progress rather than raw message text.

## 2026-07-17 — Terminal doc-sync stage not ported to the Claude path in Phase 12b
**Why:** Phase 12b brings the Phase 7 disposition matrix + convergence to the Claude dispatch path (full finding-handling **logic** parity, 0.8.0), but deliberately leaves out the runner's **terminal doc-sync stage** (reconcile existing docs to the shipped whole-plan diff after final review passes). Reason: a forge-wide **doc revamp** is coming next regardless, which will subsume ad-hoc doc-sync; porting it now would be work the revamp reworks. This is the one known Claude/Codex divergence 12b leaves — but it's a terminal reconciliation *stage*, not finding-handling logic, so the parity claim (steering-matrix + convergence identical across harnesses) holds. On Codex the runner still runs doc-sync; on Claude the orchestrator stops at deferral write-back after final review.
**From:** Phase 12b spec review (2026-07-17).
**Follow-up:** fold Claude-path doc-sync into the coming doc revamp — either port the runner's dispatched-subagent doc-sync or design a cross-harness doc-reconciliation step there. Until then, Claude runs reconcile docs by hand at completion.

## 2026-07-17 — Claude *dispatch* path never got the Phase 7 canon; dropped in the Phase 8→10–12 decomposition — RESOLVED 2026-07-17 (Phase 12b)
**Why:** Phase 7 built the disposition matrix (fix/defer/halt) **and** convergence rework, and scoped it explicitly "Codex-only (`forge-run.py`); the Claude path is Phase 8." When Phase 8 decomposed into Phases 10–12 (DECISIONS 2026-07-17), the parity audit surfaced the inline gaps (→10 vehicle, →11 finding-process) and the Claude-dispatch **commit/clean-tree + effort drift + stale reviewer prose** (→12 Half A) — but it **never re-homed the Phase 7 canon for the Claude dispatch path** into any phase. So Claude dispatch still runs old cap-2 + "finding → rework → re-review" with **neither** the disposition matrix nor convergence, while Codex dispatch (the runner) has both. The dispatch paths are themselves asymmetric, independent of Phase 11. Phase 11's "shared home" fix does **not** reach this: inline is the same prose on both harnesses (fixable once), but dispatch is two different mechanisms — Claude subagent reviewers vs. the Codex runner — with no shared prose to edit. Surfaced when Bryan questioned the spec's asserted "Claude dispatch stays cap-2" boundary (2026-07-17) — which was an unlogged gap dressed as a decision, now corrected.
**From:** Phase 11 spec review (2026-07-17); traces to Phase 7 (DECISIONS 2026-07-16, "Codex-only … Claude path is Phase 8") and the Phase 8 decomposition (DECISIONS 2026-07-17).
**Follow-up:** roadmap — **rethink Phase 12** as the overall "bring the Claude dispatch path into parity with the Codex runner": the Phase 7 disposition matrix + convergence rework on the Claude subagent-reviewer path, folded together with Phase 12's existing Half A (commit-per-task + clean-tree precondition) + the tier-recalibration effort drift + retiring the stale escalation-reviewer prose. Decide at Phase 12 kickoff whether that's one phase or splits further.
**Resolved:** split into Phase 12a (mechanical: effort drift, stale reviewer prose, Half A — done) and Phase 12b (this canon — done). 12b's shared `scripts/forge_dispose.py` gives the Claude dispatch path the identical disposition matrix + convergence the Codex runner uses, reaching cross-harness logic parity (0.8.0). See DECISIONS 2026-07-17 (Phase 12a and 12b) and `docs/forge/specs/2026-07-17-phase12b-claude-dispatch-parity-design.md`.
...
## 2026-07-13 — Mechanical rework-cap enforcement on the Claude Code path — RESOLVED 2026-07-17 (Phase 12b)
...
**Resolved:** Phase 12b (2026-07-17) extracted the disposition matrix + convergence decision into shared, tested `scripts/forge_dispose.py`; the Claude dispatch path now calls the identical logic via its CLI. The Claude decision is no longer enforced by orchestrator discipline — it's the same tested script the Codex runner calls in-process. See DECISIONS 2026-07-17 (Phase 12b) and `docs/forge/specs/2026-07-17-phase12b-claude-dispatch-parity-design.md`.
