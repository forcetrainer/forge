...
2026-08-21 (phase 14): Phase 14 added a third provenance value `in-run` and a fifth disposition `seed` to the matrix this spec defined, and made the `convergence: "resolved"` label this spec documented actually take effect; convergence rules, the four original cells, and the backstop of 5 are unchanged.
