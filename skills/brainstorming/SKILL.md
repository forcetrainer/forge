...
8. **Spec review** — a cold reviewer validates the spec against the codebase via `scripts/forge_docreview.py`. Planning does not begin until spec review passes. Discovery is a cold reviewer; a re-review after an amendment is a verification lap and may resume. An amendment re-enters scoped to its changed sections plus their references, full document as context. Hunting list: `@design-anti-patterns.md`, never inlined here. Invalid verdict: one retry naming the specific defect, then a contract error — the retry is the caller's job, not the module's, matching the reviewer verdict contract.
