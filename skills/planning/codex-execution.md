...
**Precondition — clean working tree:** every invocation (first run and resume) requires `git status --porcelain` to be empty, with `.forge/` self-ignored. A dirty tree causes a contract error (exit 1) naming the dirty paths; the human must commit or discard those changes before re-invoking. This is not the halted-task path: a halt freezes the in-progress attempt and returns the tree to the last committed checkpoint before the runner exits, so a resume finds a clean tree with no human git work required. The precondition still refuses a tree left dirty for an unrelated reason — no flag bypasses it. The runner never resets or stashes user work.
...
**Disposition matrix (`--autofix auto`):** every reviewer finding is classified on two runner-verified axes — provenance (checked against the actual diff, never trusted from the reviewer) × contract impact (`contract-breaking` only when the reviewer's `contract_ref` names a citable ref for the review — a coverage item or a declared `spec:<slug>` section; a null or non-citable ref downgrades to `improvement` regardless of the reviewer's label; `unverifiable` is its own third value). `in-diff` × `contract-breaking` → **fix** (reworked in-loop, the only auto-fixed cell); `improvement` findings (any provenance) → **defer** (logged, never fixed — no gold-plating the phase's own new code); `pre-existing` × `contract-breaking` → **halt** (a real scope decision, carries a drafted repair task); `unverifiable` findings (any provenance) → **seed**, logged and carried into the final review rather than fixed, deferred, or halted — reporting is unconditional on provenance, so an unverifiable finding is never withheld for want of a diff-side answer. `--autofix gate` skips the matrix: any finding at all halts.
...
**Coverage checklist:** before each review dispatch, the runner generates the checklist via `scripts/forge_checklist.py` (a task checklist for a task review, a final checklist for the final review) and folds it into the review packet; the reviewer's verdict must carry a `coverage` array satisfying it (Reviewer verdict contract, planning skill `SKILL.md`). A task's checklist is its own promises — `**Tests:**` cases, `**Acceptance:**` clauses, `**Global Constraints:**` — never a `**Spec:**`-named section; those are final-review-only coverage items and, per task, travel into the packet as context, not something the reviewer verdicts on. The **citable** set the runner hands the reviewer is wider than that checklist: coverage items plus the task's declared `spec:<slug>` sections, and only membership in that set — not a non-null `contract_ref` — makes a finding `contract-breaking`. At the **final** review the citable set is the whole plan's — every final-checklist id plus every task's `t<N>.t<M>` test-case id — computed once from the plan and passed on **every** lap, so neither a verification lap's reduced checklist nor the final checklist's omission of task-only test ids can reject a legitimate reference (a seeded per-task finding is replayed into the final packet still citing the test case it was raised against). Verification laps get the **reduced** checklist — only items referenced by the outstanding findings' `contract_ref` — not the full one. An empty checklist is a runner-level **SKIP**, not an error: the review dispatches without a coverage requirement and the receipt records `coverage_skipped`; `forge_checklist.py` invoked directly still raises on an empty checklist, since the tolerance belongs to the runner, not the generator. `forge_dispose.py` validates the coverage array (missing/unknown ids, an unbacked `violated` id, empty evidence, a `contract_ref` outside the citable set) alongside the verdict parse; an invalid array gets **one retry** naming the specific defect, which does not advance the attempt counter or convergence state, then a contract error on a second invalid verdict.
